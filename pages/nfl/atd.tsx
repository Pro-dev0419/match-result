...
const TABLE_COLUMNS: GridColDef[] = [
  ...
  { field: "ROI_DK", headerName: "ROI_DK", type: 'number',
    renderCell: (params) =>
    <p>
      <a href="https://sportsbook.draftkings.com/leagues/football/nfl?category=td-scorers&subcategory=anytime-td-scorer">{params.row.ROI_DK}%</a>
      <IconButton href="https://sportsbook.draftkings.com/leagues/football/nfl?category=td-scorers&subcategory=anytime-td-scorer"> <LaunchIcon />
      </IconButton>
    </p>,
...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/nfl/atd").then((o) => o.json());
        setData(res.data);
      } catch (e) {
      } finally {
        setLoading(false);
  ...
  return (
    <div className={styles.container}>
      <main className={styles.main}>
        <h1 className={styles.title}>Anytime TD</h1>
        {loading && "Loading"}
        {!loading && data && (
