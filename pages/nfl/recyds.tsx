...
const TABLE_COLUMNS: GridColDef[] = [
  ...
  { field: "RECEPTIONS", headerName: "Receptions" },
  { field: "RECEIVING YARDS", headerName: "Yards" },
...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/nfl/recyds").then((o) => o.json());
        setData(res.data);
      } catch (e) {
      } finally {
        setLoading(false);
  ...
  return (
    <div className={styles.container}>
      <main className={styles.main}>
        <h1 className={styles.title}>Receiving Yards</h1>
        {loading && "Loading"}
        {!loading && data && (
