...
const TABLE_COLUMNS: GridColDef[] = [
  { field: "PROP", headerName: "Prop", width: 300 },
  ...
  { field: "ROI_DK", headerName: "ROI_DK", type: 'number',
    renderCell: (params) =>
    <p>
      <a href="https://sportsbook.draftkings.com/leagues/hockey/nhl?category=shots-on-goal">{params.row.ROI_DK}%</a>
      <IconButton href="https://sportsbook.draftkings.com/leagues/hockey/nhl?category=shots-on-goal"> <LaunchIcon />
      </IconButton>
    </p>,
  ...
  { field: "LINE_RIV", headerName: "LINE_RIV" },
  { field: "AMERICAN_RIV", headerName: "RIV" },
  { field: "ROI_RIV", headerName: "ROI_RIV", type: 'number',
    renderCell: (params) =>
    <p>
...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/nhl/sog").then((o) => o.json());
        setData(res.data);
      } catch (e) {
      } finally {
        setLoading(false);
  ...
  return (
    <div className={styles.container}>
      <main className={styles.main}>
        <h1 className={styles.title}>Shots on Goal</h1>
        {loading && "Loading"}
        {!loading && data && (
